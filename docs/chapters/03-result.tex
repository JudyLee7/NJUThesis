...
\subsection{HBc--preS体外互作检测体系的建立思路}
...
考虑到HBc--preS相互作用可能较弱，且容易受到固定化方式、蛋白取向和洗涤步骤影响，本研究前期比较了多种ELISA构型。三类ELISA体系分别从不同角度优化互作检测：其一，直接包被HBc颗粒，再检测结合到HBc上的preS；其二，通过StrepXT定向捕获带Strep标签的preS，再加入HBc并检测结合的核心蛋白；其三，通过抗HBc Fab捕获HBc颗粒，再检测preS结合。通过比较这些体系的信号窗口、背景水平和重复性，可以判断传统固相ELISA是否适合作为后续突变体和小分子筛选平台。

\subsection{HBVCp149的制备为ELISA检测提供了核心蛋白样品}
...
\subsection{HBc直接包被ELISA显示较强非特异背景}
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.95\textwidth]{docs/figures/result/result_02_elisa_a.png}
...
\subsection{StrepXT捕获preS改善了preS呈递但未形成足够稳定的检测窗口}
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.95\textwidth]{docs/figures/result/result_03_strepXT.png}
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.95\textwidth]{docs/figures/result/result_04_elisa_b_1.png}
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.5\textwidth]{docs/figures/result/result_05_IgG D4.png}
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.95\textwidth]{docs/figures/result/result_06_elisa_b_2.png}
...
\subsection{抗core Fab捕获HBc未能进一步扩大preS依赖性信号}
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.95\textwidth]{docs/figures/result/result_07_elisa_c.png}
...
\subsection{ELISA体系不适合作为本研究的主要下游定量平台}
...
\subsection{NanoBiT体系初步建立并显示更适合检测HBc--preS相互作用}
...
\subsection{HBVCp182-SmBiT的重新纯化与活性确认}
