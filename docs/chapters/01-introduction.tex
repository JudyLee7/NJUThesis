...
\begin{figure}[htbp]
  ...
  \caption{Bruss研究中用于定位preS短线性功能区的突变设计。图中标示了linker substitution突变体I1--I11以及后续双丙氨酸突变体A1--A7的位置，为功能互补实验定位关键片段提供了构建基础。图片截取自Bruss的研究\cite{bruss1997short}。}
...
\begin{figure}[htbp]
  ...
  \hfill
  \includegraphics[width=0.36\textwidth]{figures/introduction/bruss1997_fig7_alanine_virion_assay.png}
  \caption{preS短线性区域对HBV virion formation的重要性。左图显示linker substitution互补实验：I1--I4突变体不能恢复L-negative HBV genome的virion分泌，而I5--I11突变体可以恢复分泌。右图显示双丙氨酸扫描实验：A1--A3和A5--A7突变体削弱或阻断virion形成，而突变非保守残基的A4仍保留virion formation能力。图片截取自Bruss的研究\cite{bruss1997short}。}
...
\begin{figure}[htbp]
  ...
  \caption{preS关键功能区域的序列保守性。Bruss比较了32个HBV isolates，并以灰色区域标示Asn98--Ser124附近的保守片段；该区域的保守性与linker substitution和双丙氨酸突变实验中的virion formation表型相互支持。图片截取自Bruss的研究\cite{bruss1997short}。}
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.92\textwidth]{figures/introduction/makbul2021_tx100_hbc_pocket.png}
  \par\medskip
  \includegraphics[width=0.88\textwidth]{figures/introduction/makbul2021_tx100_hbc_detail.png}
  \caption{Triton X-100结合HBc二聚体疏水口袋的结构证据。上图显示TX100相关密度位于HBc二聚体界面附近，并与F97等局部构象变化相关；下图从不同视角展示TX100被链C和链D形成的特定口袋容纳。图片截取自Makbul等的研究\cite{makbul2021binding}。}
...
\begin{figure}[htbp]
  ...
  \caption{香叶醇（geraniol）结合HBc中心疏水口袋及其与TX100结合位点的比较。化学结构、ITC结合曲线、冷冻电镜密度和口袋剖面共同支持geraniol可与HBc核心蛋白直接相互作用，并结合在可容纳疏水分子的特定口袋中。图片截取自Khayenko等的研究\cite{khayenko2024induction}。}
...
既往小分子筛选研究已经证明，阻断核心蛋白与表面蛋白之间的相互作用具有可行性。Asif-Ullah等通过ELISA检测HBc与preS结合并筛选化合物，发现部分小分子能够抑制该相互作用，并降低细胞中HBV颗粒释放水平\cite{asifullah2006compounds}。虽然这些早期化合物距离临床药物仍有较大距离，但它们从概念上证明了核衣壳--包膜蛋白界面具有可干预性。TX100和geraniol的HBc口袋结合研究\cite{makbul2021binding,khayenko2024induction}则进一步提示，后续药物设计可以围绕HBc表面可成药口袋、竞争机制和体外互作验证进行理性优化。
